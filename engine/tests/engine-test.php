...
$cases = [
    ...
    // A silent no-op is the failure worth catching: an author who writes a
    // drift on a ledger that is not a daily system is sure it is climbing.
    ['a drift on a ledger nothing moves',
        mutate($T, ['economies', 0, 'daily'], ['drift' => 1]),
        'economies[0].daily is set but daily_system is not true'],
    ['a drift that is not a number',
    ...
    ['a floor above its own ceiling',
        ...
        'economies[1].daily.floor (9) is above the ceiling (2)'],
...
// ---------------------------------------------------------------------------
// QR: the one thing between a xeric and the phone in somebody's pocket. The
// only test that means anything is module-for-module against a real encoder —
// a QR that is almost right is a QR that does not scan — so the matrices in
// here were generated by an independent reference implementation and pasted.
// ---------------------------------------------------------------------------
...
$qrRows = fn(string $s): array => array_map(fn($r) => implode('', $r), xeric_qr_matrix($s));

// Version 1 (21 modules) — the reference's own output for this exact string.
$qrRef1 = ['111111100110001111111', '100000100000001000001', '101110101110101011101',
           '101110100010001011101', '101110100110001011101', '100000100110001000001',
           '111111101010101111111', '000000001101100000000', '111011111011011000100',
           '010101000011101110001', '011010101010110010111', '111001010100100010010',
           '000111100110110101000', '000000001001111110011', '111111101111101010111',
           '100000101110000110000', '101110101110000001001', '101110100001100011000',
           '101110101111111010101', '100000101111100010010', '111111101110110011011'];
ok('qr: a version-1 code is exact, module for module',
    $qrRows('https://xeric.dev') === $qrRef1, implode("\n", $qrRows('https://xeric.dev')));

ok('qr: it grows a version when the text does, and stays square',
    count($qrRows('http://192.168.1.34:8787/play.php?w=milldale')) === 29
    && count($qrRows('http://10.0.0.5:8787/')) === 25
    && count($qrRows('http://192.168.100.200:9000/play.php?w=a-very-long-world-slug-here')) === 33);

$qrM = xeric_qr_matrix('http://10.0.0.5:8787/');
ok('qr: the finders are in three corners and never the fourth',
    $qrM[0][0] === 1 && $qrM[0][6] === 1 && $qrM[6][0] === 1
    && $qrM[0][24] === 1 && $qrM[24][0] === 1
    && $qrM[3][3] === 1 && $qrM[1][1] === 0);
ok('qr: the timing patterns alternate along row six and column six',
    $qrM[6][8] === 1 && $qrM[6][9] === 0 && $qrM[8][6] === 1 && $qrM[9][6] === 0);
ok('qr: the dark module is where the spec puts it and nowhere else',
    $qrM[count($qrM) - 8][8] === 1);
ok('qr: the same text encodes identically every time — a code is not a guess',
    $qrRows('http://10.0.0.5:8787/') === $qrRows('http://10.0.0.5:8787/'));

$qrSvg = xeric_qr_svg('http://10.0.0.5:8787/', 4);
ok('qr: the svg is self-contained, sized, and carries no script',
    str_starts_with($qrSvg, '<svg') && str_contains($qrSvg, 'viewBox')
    && !str_contains($qrSvg, '<script') && str_contains($qrSvg, 'role="img"'));
ok('qr: and text nobody could fit is refused rather than silently truncated',
    err(fn() => xeric_qr_matrix(str_repeat('x', 400))) !== '');
